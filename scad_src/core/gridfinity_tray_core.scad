/*
 * Gridfinity tray core (shared geometry + Gridfinity base + outer walls).
 *
 * This file intentionally does NOT define user-facing parameters.
 * Variant entrypoints should:
 * - define parameters (grid size, height, wall style, base options, etc)
 * - include this file
 * - include an interior module file that defines:
 *     - `positions` (list of [x,y] in opening coordinates)
 *     - `build_holders()` and `build_raised_floor()` (or no-op versions)
 * - call `main();`
 */
...
// Usable area for placement (accounting for outer tray wall)
...
// Generic interior helpers (shared across interior types).
...
// Inner opening (the space inside the tray wall).
...
// Starting position offset (positions are in opening coordinates [0..opening_*])
...
// ===== Modules (Gridfinity base library) ===== //
